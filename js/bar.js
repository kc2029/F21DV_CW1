...
barChart.appendChild(closeButton);
...
chart.addEventListener('mousedown', (e) => {
  ...
  startX = e.clientX;
  startY = e.clientY;
  translateX = parseFloat(window.getComputedStyle(chart).getPropertyValue('transform').split(', ')[4]);
  translateY = parseFloat(window.getComputedStyle(chart).getPropertyValue('transform').split(', ')[5]);
});
...
chart.addEventListener('mousemove', (e) => {
  if (isDragging) {
    ...
    chart.style.transform = `translate(${translateX + deltaX}px, ${translateY + deltaY}px)`;
  }
});
